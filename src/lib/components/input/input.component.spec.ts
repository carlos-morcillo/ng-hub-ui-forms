...
describe('HubInputComponent', () => {
	...
	describe('input group addons', () => {
		/** Host exercising the prepend / append addon slots of the input group. */
		@Component({
			standalone: true,
			imports: [HubInputComponent, ReactiveFormsModule],
			template: `<hub-input [formControl]="ctrl" [prepend]="prepend" [append]="append" />`
		})
		...
		let addonFixture: ComponentFixture<AddonHostComponent>;
		let addonHost: AddonHostComponent;

		const group = (): HTMLElement =>
			addonFixture.nativeElement.querySelector('hub-input')!.querySelector('.hub-input__group');

		beforeEach(async () => {
			TestBed.resetTestingModule();
			await TestBed.configureTestingModule({
				imports: [AddonHostComponent, ReactiveFormsModule]
			}).compileComponents();

			addonFixture = TestBed.createComponent(AddonHostComponent);
			addonHost = addonFixture.componentInstance;
			addonFixture.detectChanges();
		});

		it('renders a prepend addon and flags the group so the control can drop its leading radius', () => {
			addonHost.prepend = '€';
			addonFixture.detectChanges();

			expect(group().querySelector('.hub-input__addon--prepend')?.textContent).toContain('€');
		...
		it('renders an append addon and flags the group so the control can drop its trailing radius', () => {
			addonHost.append = '€';
			addonFixture.detectChanges();

			expect(group().querySelector('.hub-input__addon--append')?.textContent).toContain('€');
			expect(group().classList.contains('hub-input__group--has-append')).toBe(true);
			expect(group().classList.contains('hub-input__group--has-prepend')).toBe(false);
		});

		it('flags both sides when prepend and append are combined', () => {
			addonHost.prepend = 'https://';
			addonHost.append = '.com';
			addonFixture.detectChanges();
		...
		it('flags neither side when no addon is configured', () => {
			expect(group().classList.contains('hub-input__group--has-prepend')).toBe(false);
			expect(group().classList.contains('hub-input__group--has-append')).toBe(false);
		});

		it('renders every entry of an addon array as consecutive siblings', () => {
			addonHost.prepend = ['a', 'b'];
			addonHost.append = ['x', 'y'];
			addonFixture.detectChanges();

			expect(group().querySelectorAll('.hub-input__addon--prepend')).toHaveLength(2);
			expect(group().querySelectorAll('.hub-input__addon--append')).toHaveLength(2);
		});

		it('keeps the affix spans between the addons and the control', () => {
			// Pins the reason the corner flattening is driven by group modifiers instead of
			// sibling combinators: the affixes are always rendered, so an addon is never the
			// adjacent sibling of the control and `+` would silently never match.
			addonHost.prepend = '€';
			addonHost.append = '€';
			addonFixture.detectChanges();

			const control = group().querySelector('.hub-field__control')!;

			expect(control.previousElementSibling?.classList.contains('hub-input__addon--prepend')).toBe(false);
			expect(control.nextElementSibling?.classList.contains('hub-input__addon--append')).toBe(false);
		});
	});
